...
public class M_AIManager : MonoBehaviour
{
    ...
    public M_GameManager.Player aiPlayerColor = M_GameManager.Player.None;
    ...
    private void Start()
    {
        ...
        // [NET][FIX] 멀티플레이 모드라면 UI 숨기고 즉시 비활성화 (가장 강력한 차단)
        if (gameManager != null && (gameManager.currentMode == M_GameManager.GameMode.MultiPlay || PhotonNetwork.InRoom))
        {
            Debug.Log("[AIManager] 멀티플레이 환경 감지 - AI를 비활성화합니다.");
            if (aiStateText != null) aiStateText.text = "";
    ...
    public void SetAIColor(M_GameManager.Player color)
    {
        // 멀티플레이 모드라면 무시
        if (gameManager != null && gameManager.currentMode == M_GameManager.GameMode.MultiPlay) return;
    ...
    private void OnTurnChanged(M_GameManager.Player currentPlayer)
    {
        // 멀티플레이 모드라면 무시
        if (gameManager != null && (gameManager.currentMode == M_GameManager.GameMode.MultiPlay || PhotonNetwork.InRoom))
        {
        ...
        if (aiPlayerColor == M_GameManager.Player.None) return;
    ...
    private IEnumerator CalculateAndMakeMove()
    {
        ...
        {
            if (aiPlayerColor == M_GameManager.Player.Black && gameManager.IsForbidden(move.x, move.y, aiPlayerColor))
                continue;
    ...
    private int Minimax(int[,] board, int depth, int alpha, int beta, bool isMaximizing)
    {
        ...
        if (isMaximizing)
        {
            ...
            foreach (var move in candidates)
            {
                if (aiPlayerColor == M_GameManager.Player.Black && IsSimulateForbidden(board, move.x, move.y, (int)aiPlayerColor)) continue;
        ...
        else
        {
            ...
            int opponentColor = (int)((aiPlayerColor == M_GameManager.Player.Black) ? M_GameManager.Player.White : M_GameManager.Player.Black);

            foreach (var move in candidates)
            {
                if (opponentColor == (int)M_GameManager.Player.Black && IsSimulateForbidden(board, move.x, move.y, opponentColor)) continue;
    ...
    private bool IsBoardEmpty()
    {
        ...
        for (int x = 0; x < size; x++)
        {
            for (int y = 0; y < size; y++)
            {
                if (gameManager.GetCellState(x, y) != M_GameManager.Player.None) return false;
            }
        }
        return true;
    }
